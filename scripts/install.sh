...
echo "  4. Copy $(find "${BUILD_MD_ROOT}/templates/rules" -maxdepth 1 -name '*.md' 2>/dev/null | wc -l | tr -d ' ') rule files → .agents/rules/"
echo "  5. Copy $(find "${BUILD_MD_ROOT}/templates/schemas" -maxdepth 1 -name '*.json' 2>/dev/null | wc -l | tr -d ' ') schema files → meta/schemas/"
echo "  6. Copy $(find "${BUILD_MD_ROOT}/scripts" -maxdepth 1 -name '*.py' 2>/dev/null | wc -l | tr -d ' ') Python scripts → scripts/"
echo "  7. Copy applets/dashboard.html, principles/PRINCIPLES.md → .agents/"
echo "  8. Copy $(find "${BUILD_MD_ROOT}/commands" -maxdepth 1 -name '*.md' 2>/dev/null | wc -l | tr -d ' ') slash commands → .claude/commands/"
